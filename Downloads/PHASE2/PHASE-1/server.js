...
const KOHA_CONFIG = {
  ...
  kioskEmailEndpoint: process.env.KOHA_KIOSK_PLUGIN_EMAIL_ENDPOINT || '/api/v1/contrib/kiosk/email-receipt',
  noticeCodeRenewReceipt: process.env.KOHA_NOTICE_CODE_RENEW_RECEIPT || 'KIOSK_RENEW_RECEIPT'
};
...
async function handleEmailReceipt(req, res) {
  const requestId = uuidv4();
  try {
    const body = await parseRequestBody(req);
    const { type, patron, results } = body;

    if (!patron || !patron.borrowernumber) {
      sendJson(res, 400, { ok: false, errorCode: 'UNKNOWN_ERROR', message: 'Patron borrower number is required' });
      return;
    }

    if (type !== 'renew') {
      sendJson(res, 400, { ok: false, errorCode: 'UNKNOWN_ERROR', message: 'Only renew receipts are currently supported' });
      return;
    }

    // Strip /api/v1 or similar off the baseUrl to append the custom plugin endpoint cleanly
    const rootUrl = String(KOHA_CONFIG.baseUrl).replace(/\/api\/v1\/?$/i, '');
    const pluginUrlPath = KOHA_CONFIG.kioskEmailEndpoint.startsWith('/') ? KOHA_CONFIG.kioskEmailEndpoint : '/' + KOHA_CONFIG.kioskEmailEndpoint;
    const fullUrl = `${rootUrl}${pluginUrlPath}`;

    const pluginPayload = {
      borrowernumber: patron.borrowernumber,
      notice_code: KOHA_CONFIG.noticeCodeRenewReceipt,
      results: results || [],
      kiosk_device: 'self-checkout'
    };

    logger.info('Renew', `[EMAIL RECEIPT] Queuing notice for borrowernumber=${patron.borrowernumber}`, { requestId, fullUrl });
    ...
    const pluginResponse = await new Promise((resolve, reject) => {
      ...
      request.write(JSON.stringify(pluginPayload));
      request.end();
    });

    if (pluginResponse.statusCode >= 200 && pluginResponse.statusCode < 300) {
      let parsed = {};
      try { parsed = JSON.parse(pluginResponse.body); } catch(e){}
      logger.info('Renew', `[EMAIL RECEIPT SUCCESS] ${pluginResponse.statusCode}`, { requestId });
      sendJson(res, 200, { ok: true, status: parsed.status || 'queued' });
    } else {
      logger.warn('Renew', `[EMAIL RECEIPT FAIL] Status: ${pluginResponse.statusCode} Body: ${pluginResponse.body}`, { requestId });
      
      let errorCode = 'UNKNOWN_ERROR';
      if (pluginResponse.statusCode === 404 && /email|address/i.test(pluginResponse.body)) {
         errorCode = 'NO_EMAIL_ON_FILE';
      } else if (pluginResponse.statusCode === 401 || pluginResponse.statusCode === 403) {
         errorCode = 'KOHA_AUTH_FAILED';
      } else if (pluginResponse.statusCode === 404) {
         errorCode = 'NOTICE_TEMPLATE_MISSING';
      }
      sendJson(res, 200, { ok: false, errorCode, message: `Server returned ${pluginResponse.statusCode}` });
    }

  } catch (error) {
    logger.error('Renew', `[EMAIL RECEIPT EXCEPTION] ${error.message}`, { requestId });
    let errorCode = 'UNKNOWN_ERROR';
    if (/network|connect|timeout/i.test(error.message)) {
      errorCode = 'KOHA_UNREACHABLE';
    }
    sendJson(res, 200, { ok: false, errorCode, message: error.message });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && reqUrl.pathname === '/api/receipt/email') {
    await handleEmailReceipt(req, res);
    return;
  }
